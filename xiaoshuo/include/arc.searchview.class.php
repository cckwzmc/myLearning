...
class SearchView
{
	...
	function CountRecord()
	{
		...
		$ksqls[] = " arc.arcrank > -1 and arc.isbookpage is null";
		...
		for($i=0;$i<$row;$i++)
		{
			...
			for($j=0;$j<$col;$j++)
			{
				...
				if($row = $this->dsql->GetArray("al"))
				{
					...
					if(!empty($row["isbookpage"])){
						$arcu=array($row["id"],"detail",$bookid);
					}else{
						$arcu=array($row["id"],"page",$bookid);
					}
					$row["arcurl"] = GetFileUrl($arcu,$row["typeid"],$row["senddate"],$row["title"],
