...
class ListView
{
	...
	function CountRecord()
	{
		...
		else
		{
			...
			{
				if($this->CrossID=='') {
					if("table"==$this->a_xstype)
					{
						$this->addSql .= " And ( a.isbookpage in (".GetSonIds($this->TypeID,$this->Fields['channeltype']).") ) ";
					}else{
				...
				else 
				{
					if("table"==$this->a_xstype)
					{
						$this->addSql .= " And ( a.isbookpage in (".GetSonIds($this->TypeID,$this->Fields['channeltype']).",{$this->CrossID}) ) ";
					}else{
...
//				if("table"==$this->a_xstype)
//				{
//					$this->addSql.=" And a.flag like'%x%' And a.isbookpage is not null ";
//				}
			}
			else
			{
		...
		{
			$condi_query='';
			if(empty($this->a_xstype)){
				$condi_query=" And a.flag like '%t%' and a.isbookpage is null ";
			}else{
				$condi_query=" And a.flag like '%x%' and a.isbookpage is not null ";
			}
			$cquery = "Select count(*) as dd From `#@__arctiny` arc left join `#@__archives` a on arc.id=a.id where ".$this->addSql.$condi_query;
		...
		if("table"==$this->a_xstype)
		{
			$templisttable=	$this->TypeLink->TypeInfos['templisttable'];
		}else{
			$templisttable=	$this->TypeLink->TypeInfos['templist'];
		}
		$templisttable= str_replace("{style}","default",$templisttable);
		$tempfile = $GLOBALS['cfg_basedir'].$GLOBALS['cfg_templets_dir']."/".$templisttable;
		...
		if(!is_object($ctag)&&"table"!=$this->a_xstype)
		{
			$this->PageSize = 20;
		}
		else if("table"!=$this->a_xstype)
		{
...
//			$makeFile = str_replace("{page}",$this->PageNo,$makeFile);
			$typedir = MfTypedir($this->Fields['typedir']);
			$makeFile=$typedir."/".(!empty($this->Fields['isbookpage'])?($this->Fields['isbookpage']):($this->Fields['id']))."/table.html";
			$murl = $makeFile;
			if(!ereg("^/",$makeFile))
			{
				$makeFile = "/".$makeFile;
	...
	function ParseDMFields($PageNo,$ismake=1)
	{
		...
		foreach($this->dtp->CTags as $tagid=>$ctag)
		{
			if($ctag->GetName()=="list")
			{
				...
				$tmparg=$this->a_xstype;
				if($tmparg=='table'){
					$firstarg=Array($limitstart,"table");
				}else{
					$firstarg=Array($limitstart,$ctag->GetAtt("bookpage"));
				}
				$this->dtp->Assign($tagid,$this->GetArcList($firstarg,$row,$ctag->GetAtt("col"),$ctag->GetAtt("titlelen"),
				$ctag->GetAtt("infolen"),
	...
	//获得一个单列的文档列表
	function GetArcList($limitstart,$row=10,$col=1,$titlelen=30,$infolen=250,
	$imgwidth=120,$imgheight=90,$listtype="all",$orderby="default",$innertext="",$tablewidth="100",$ismake=1,$orderWay='desc')
	{
		global $cfg_list_son;
		$a_limitstart=0;
		$a_xstype='';
		if(is_array($limitstart)){
			$a_limitstart=$limitstart[0];
			$a_xstype=$limitstart[1];
		}else{
			$a_limitstart=$limitstart;
		}
		$typeid=$this->TypeID;
		
		if($row=='') $row = 10;
		if($a_limitstart=='') $a_limitstart = 0;
	...
	$add_xs="";
			 if("LIST"==$a_xstype){
			 $add_xs=" And arc.isbookpage is null ";
			 }
		   ...
		   $addJoin
  where {$this->addSql} $add_xs $ordersql limit $a_limitstart,$row";
		}
		...
		else
		{
			...
			$ids = array();
			if("table"==$a_xstype){
				$query = "Select id From `#@__archives` arc where {$this->addSql} $ordersql ";
				$query=str_replace("a.isbookpage","arc.isbookpage",$query);
			} else if("LIST"==$a_xstype){
				$query = "Select id From `#@__archives` arc where {$this->addSql} and isbookpage is null $ordersql limit $a_limitstart,$row ";
				$query=str_replace("a.isbookpage","arc.isbookpage",$query);
			}
			else{
				$query = "Select id From `#@__arctiny` arc where {$this->addSql} $ordersql limit $a_limitstart,$row ";
			}
			...
			else
			{
				$forbooklist='';
				if("table"==$a_xstype){
					$forbooklist =" arc.flag like'%x%' and arc.isbookpage is not  null and ";
				}else
				if(!empty($a_xstype)){
					$forbooklist =" arc.flag like'%t%' and arc.isbookpage is  null and ";
				}
				$query = "Select arc.*,tp.typedir,tp.typename,tp.corank,tp.isdefault,tp.defaultname,
		   			tp.namerule,tp.namerule2,tp.ispart,tp.moresite,tp.siteurl,tp.sitepath,arc.lasterartid 
		   			$addField
		   			from `#@__archives` arc left join `#@__arctype` tp on arc.typeid=tp.id
		   			$addJoin
		   			where $forbooklist arc.id in($idstr) $ordersql ";
			}
			$t2 = ExecTime();
			//echo $t2-$t1;
		...
		for($i=0;$i<$row;$i++)
		{
			...
			for($j=0;$j<$col;$j++)
			{
				if($row = $this->dsql->GetArray("al"))
				{
					...
					$firstarg;
					if(!empty($a_xstype)&&"table"==$a_xstype){
						$bookid="";
						if(empty($row['isbookpage'])){
							$bookid=$row['id'];
						}else{
							$bookid=$row['isbookpage'];
						}
						$firstarg=Array($row['id'],"detail",$bookid);						
					}else if(!empty($a_xstype)){
						$bookid="";
						if(empty($row['isbookpage'])){
							$bookid=$row['id'];
						}else{
						...
						$firstarg=Array($row['id'],"page",$bookid);
					}else{	
						$firstarg=$row['id'];
					}
					$row['filename'] = $row['arcurl'] = GetFileUrl($firstarg,$row['typeid'],$row['senddate'],$row['title'],$row['ismake'],
