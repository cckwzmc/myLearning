...
function GetFileName($aid,$typeid,$timetag,$title,$ismake=0,$rank=0,$namerule='',$typedir='',$money=0,$filename='')
{
	...
	if($rank!=0 || $ismake==-1 || $typeid==0 || $money>0)
	{
		...
		if($cfg_rewrite == 'Y')
		{
			return $GLOBALS["cfg_plus_dir"]."/view-".$c_aid.'-1.html';
		}
		else
		{
			return $GLOBALS['cfg_phpurl']."/view.php?aid=$c_aid";
		}
	}
	else
	{
		/**
		 * TODO 这里是创建文件目录的地方，以后要生成有好的文件目录可以修改这里
		 * 目录结构说明
		 * 栏目/书名(目录，封面)/书名(章节页面)
		 * @var unknown_type
		 */
		...
		$articleUrl="";
		if($c_xstype=='page'){
			$articleUrl=$articleDir."/".$bookid."/index.html";
		}else if($c_xstype=='table'){
			$articleUrl=$articleDir."/".$bookid."/table.html";
		}else if($c_xstype=='detail'){
			$articleUrl=$articleDir."/".$bookid."/$c_aid.html";
		}else{
			...
			$arr_rpvalues = array($articleDir,$y, $m, $d, $timetag, $c_aid, dd2char($m.$d.$c_aid.$y));
			if($filename != '')
			{
			...
			{
				$articleRule = str_replace('{pinyin}',GetPinyin($title).'_'.$c_aid,$articleRule);
				$articleRule = str_replace('{py}',GetPinyin($title,1).'_'.$c_aid,$articleRule);
			}
