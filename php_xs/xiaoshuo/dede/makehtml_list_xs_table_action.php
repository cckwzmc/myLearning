...
if(!empty($tid))
{
	if(!empty($xstype)){
		if(!isset($_Ts[$tid]))
		{
			showmsg('没有该小说数据, 可能缓存文件(/data/cache/inc_catalog_base.inc)没有更新, 请检查是否有写入权限');
			exit();
		}
	}else{
		if(!isset($_Cs[$tid]))
	...
	if($_Cs[$tid][1]>0||!empty($xstype))
	{
		 require_once(DEDEINC."/arc.listview.class.php");
		 $tarr=array($tid,$xstype);
		 $lv = new ListView($tarr);
...
else
{
	if($finishType)
	{
		$gourl = "makehtml_list_xs_table_action.php?gotype={$gotype}&uppage=$uppage&maxpagesize=$maxpagesize&typeid=$typeid&pageno=$nextpage&xstype=$xstype";
		ShowMsg("成功创建栏目：".$tid."，继续进行操作！",$gourl,0,100);
		exit();
	}
	else
	{
		$gourl = "makehtml_list_xs_table_action.php?gotype={$gotype}&uppage=$uppage&mkpage=$mkpage&maxpagesize=$maxpagesize&typeid=$typeid&pageno=$pageno&xstype=$xstype";
