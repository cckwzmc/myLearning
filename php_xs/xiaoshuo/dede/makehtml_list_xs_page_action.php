...
if($totalnum > 500 && empty($typeid)) {
	$dsql->Execute('out',"Select a.id From `#@__arctiny`   a left join  `#@__archives` ar on a.id=ar.id  $idsql order by typeid asc $limitSql");
}
else {
...
if($tjnum < $totalnum)
{
	$nurl  = "makehtml_list_xs_page_action.php?endid=$endid&startid=$startid&typeid=$typeid&xstype=$xstype";
...
else
{
	if($typeid!='')
	{
		  ShowMsg("生成文件：$totalnum 总用时：{$ttime} 分钟，现转向当前栏目更新&gt;&gt;","makehtml_list_xs_page_action.php?typeid=$typeid&uptype=all&maxpagesize=50&upnext=1&xstype=$xstype");
	}
	else
	{
		if($uptype=='') {
			ShowMsg($xstype."完成所有创建任务！，生成文件：$totalnum 总用时：{$ttime} 分钟。","javascript:;");
		}
		else {
