	...
	function GetPreNext($gtype='')
	{
		...
		if(count($this->PreNext)<2)
		{
			...
			if(is_array($preRow))
			{
				...
				$this->PreNext['pre'] = "<a href='$mlink'>上一页</a> ";
			}
			else
			{
				$this->PreNext['pre'] = "上一页：没有了 ";
			}
			$this->PreNext['pre'].="<a href='table.html'>返回目录</a>";
			$bookid=$preRow['isbookpage'];
			$this->PreNext['pre'].="<a href='/plus/recommend.php?aid=$bookid'>推荐本书</a>";
			$this->PreNext['pre'].="<a href='/plus/stow.php?aid=$aid'>加入书签</a>";
			if(is_array($nextRow))
			{
				$page_aid;
				$bookid="";
				...
				$this->PreNext['next'] = "<a href='$mlink'>下一页</a> ";
			}
			else
			{
				$this->PreNext['next'] = "下一页：没有了 ";
			}
		}
		if($gtype=='pre')
		{
