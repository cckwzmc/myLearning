...
function lib_arclist(&$ctag,&$refObj)
{
	...
	//列出书名
    $bookpage =$ctag->GetAtt('bookpage');
    //列出章节
    $booklist=$ctag->GetAtt('booklist');
	...
	return lib_arclistDone
	       (
	         ...
	         $ctag->GetAtt('limit'), $flag,$ctag->GetAtt('orderway'), $ctag->GetAtt('subday'), $ctag->GetAtt('noflag'),$booklist,$bookpage
	       );
}

function lib_arclistDone(&$refObj, &$ctag, $typeid=0, $row=10, $col=1, $titlelen=30, $infolen=160,
        $imgwidth=120, $imgheight=90, $listtype='all', $orderby='default', $keyword='',
        $innertext='', $arcid=0, $idlist='', $channelid=0, $limit='', $att='', $order='desc', $subday=0, $noflag='',$booklist='',$bookpage='')
{
	...
	if($idlist=='')
	{
		...
		if(!empty($bookpage)) $orwheres[]=" And arc.isbookpage is null";
	...
	# TODO 这里是小说目录更新
	if(!empty($bookpage)){
		   $orwhere=!empty($orwhere)?str_replace("where"," and ",$orwhere):$orwhere;
			$query = "Select arc.*,tp.typedir,tp.typename,tp.corank,tp.isdefault,tp.defaultname,tp.namerule,
			tp.namerule2,tp.ispart,tp.moresite,tp.siteurl,tp.sitepath from $addfieldsSql
			 `$maintable` arc left join `#@__arctype` 
			tp on arc.typeid=tp.id where arc.flag like '%t%' and isbookpage is null  
			$addfieldsSqlJoin $orwhere $ordersql $limitsql";
	}else if(!empty($booklist))
	{		$orwhere=!empty($orwhere)?str_replace("where"," and ",$orwhere):$orwhere;
			$query = "Select arc.*,tp.typedir,tp.typename,tp.corank,tp.isdefault,tp.defaultname,tp.namerule,
			tp.namerule2,tp.ispart,tp.moresite,tp.siteurl,tp.sitepath from $addfieldsSql `$maintable` arc left join `#@__arctype` 
			tp on arc.typeid=tp.id where arc.flag like '%x%' and isbookpage is not null  
			$addfieldsSqlJoin $orwhere $ordersql $limitsql ";
	}else{
			$query = "Select arc.*,tp.typedir,tp.typename,tp.corank,tp.isdefault,tp.defaultname,tp.namerule,
	...
	for($i=0; $i<$line; $i++)
	{
		...
		for($j=0; $j<$col; $j++)
		{
			...
			if($row = $dsql->GetArray("al"))
			{
				...
				$firstarg=Array($row['id'],$arc_type,$bookid);
