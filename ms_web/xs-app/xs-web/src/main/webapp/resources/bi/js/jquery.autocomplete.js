...
(function($) {
	...
	Autocomplete.prototype = {
		...
		initialize : function() {
			...
			$(
					'<div id="'
							...
							+ '" style="position:absolute;z-index:9999;"><div class="autocomplete-shadow"><div class="autocomplete" id="'
							+ autocompleteElId
		...
		processResponse : function(text) {
			var response;
			try {
				response = jsonGet(text)
			} catch (err) {
				return
			}
			if (!this.options.noCache) {
				this.cachedResponse[response.term] = response;
				if (response.key.length === 0) {
					this.badQueries.push(response.term)
				}
			}
			if (this.getQuery(this.currentValue).length > 0) {
				this.suggestions = response.key;
