...
import com.iris.scholar.system.service.mock.MockNodeValidator;
 ...
 * @author DigitalSonic
 */
public class ValidationTask implements Callable<SOAPMessage> {
    ...
    public SOAPMessage call() throws Exception {
        SOAPMessage node = ValidationService.NODE_MAP.get(wsdl);
        Lock lock = null;
        ...
        if (node != null) {
            ...
            if (lock.tryLock()) {
                ...
                try {
                    SOAPMessage result = MockNodeValidator.validateNode(wsdl);
     ...
     */
    private SOAPMessage mergeNode(SOAPMessage src, SOAPMessage dest) {
